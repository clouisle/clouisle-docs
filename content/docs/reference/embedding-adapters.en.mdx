...
| --- | --- | --- | --- |
| **`OpenAICompatibleEmbeddingAdapter`** | `openai`, `openai_responses`, `deepseek`, `moonshot`, `zhipu`, `qwen`, `baichuan`, `minimax`, `volcengine`, `siliconflow`, `xai`, `ollama`, `custom` | Issues direct HTTP POST requests to the `/embeddings` endpoint of the resolved Base URL (standard OpenAI protocol) | Prefers `usage.total_tokens` returned by the upstream API; falls back to local `tiktoken` tokenization if omitted |
| **`FallbackEmbeddingAdapter`** | `google`, `azure_openai`, and other providers | Invokes specialized LangChain wrappers (e.g. `GoogleGenerativeAIEmbeddings`, `AzureOpenAIEmbeddings`) via `aembed_documents` | Returns an empty `Usage` object; outer `team_embedding` counts tokens locally using `tiktoken` |

<Callout type="warn" title="Anthropic Limitation">
Anthropic does not offer a standalone text embedding API. In Clouisle, attempting to use an Anthropic model with `model_type: embedding` will raise `ValueError: Unsupported provider for embedding: anthropic` at runtime.
</Callout>
...
- **Locked Model**: Once a Knowledge Base is created, its `embedding_model_id` is **permanently immutable**. Updates via `PATCH /knowledge-bases/{id}` reject or ignore any attempts to alter `embedding_model_id`.
