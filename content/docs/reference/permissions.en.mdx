...
The backend initializes 5 global built-in roles at startup with the following default permissions:
...
| **Admin** | Global built-in | Includes `admin:dashboard:access` and all administrative and operational permissions for resources, but excludes create/update/delete of system roles/permissions themselves and modification of global site settings | Workspace administrator, technical lead |
| **Team Admin** | Global built-in | Team-level administrator role; has all Member permissions plus updating and managing the team itself (`team:update`, `team:manage`) | Team lead, project lead |
...
- `owner` and `admin` map to the `Team Admin` or `Admin` permission set at team scope
