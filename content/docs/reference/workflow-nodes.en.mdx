...
<Callout type="info" title="Image needed: Node configuration reference">
  File: `/images/workflow-node-reference.png`. Screenshot content: the workflow node panel and three representative configuration drawers; the image should annotate model, logic, and transform node fields respectively. Crop the screenshot to the relevant areas and outline key actions with the theme color. Future image `alt` and caption will both use "Node configuration reference".
</Callout>
