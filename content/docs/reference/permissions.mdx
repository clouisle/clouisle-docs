...
后端在系统启动时初始化 4 个全局内置角色，各角色拥有的默认权限如下：

| 角色名称 | 角色类型 | 核心权限特征 | 典型适用人员 |
| --- | --- | --- | --- |
| **Super Admin** | 全局内置 | 拥有通配权限码 `*`，完全绕过权限限制，可执行系统配置、角色管理、审计导出等所有操作 | 运维负责人、系统所有者 |
| **Admin** | 全局内置 | 包含 `admin:dashboard:access` 与所有资源的管理类与普通操作权限，但不包含系统角色/权限自身的增删改、以及修改全局站点设置 | 工作空间管理员、团队技术主管 |
...
- `owner` 与 `admin` 对应团队作用域下的 `Admin` 角色
