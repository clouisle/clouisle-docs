...
<Callout type="info" title="图片待补充：按权限过滤的管理导航">
  文件：`/images/permission-guarded-navigation.png`。截图内容：不同权限用户看到的工作台侧栏对比，标出被隐藏的管理项；截图不得展示真实用户数据。截图需裁剪到相关区域，并用主题色框线标出关键操作。未来图片的 `alt` 与 caption 均使用“按权限过滤的管理导航”。
</Callout>
