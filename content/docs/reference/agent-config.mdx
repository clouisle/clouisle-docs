...
<Callout type="info" title="图片待补充：Agent 配置字段总览">
  文件：`/images/agent-config-reference.png`。截图内容：Agent 编辑器编排页与设置抽屉拼接图，标出本页覆盖的字段组；正式截图需分成基础、知识/工具、记忆/媒体三张图。截图需裁剪到相关区域，并用主题色框线标出关键操作。未来图片的 `alt` 与 caption 均使用“Agent 配置字段总览”。
</Callout>
