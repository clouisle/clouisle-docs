...
系统每秒收集硬件指标，并在 Redis 列表中保留最近 120 个健康快照（有效期 24 小时，通过 `GET .../system/trend` 可获取快照趋势）。健康状态阈值与指标如下：
...
- **架构考量**：Tortoise 共享连接池默认 `maxsize=5`，PostgreSQL 默认最大连接数 `max_connections=100`。在多进程（如 Gunicorn / Uvicorn 运行 17 个 Worker 进程）部署下，将单个聚合并发限制为 `4`，能确保任意时刻聚合分析都不会打满数据库连接。
