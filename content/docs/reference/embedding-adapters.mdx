...
<Callout type="warn" title="不支持的供应商">
...
### 2. 维度与模型的不可变性约束
- **模型不可变**：知识库一旦创建，`embedding_model_id` 即被**永久锁定**。更新知识库接口（`PATCH /knowledge-bases/{id}`）检测到变更 `embedding_model_id` 时会**拒绝**并返回校验错误（`embedding_model_locked_after_kb_creation`）。
