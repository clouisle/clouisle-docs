...
| POST | `/api/v1/admin/models/discover` | 发现提供商可用模型 | admin:model:create |
...
完整提供商清单（`code` 即创建模型时的 `provider` 取值）：

| `code` | `name` | 默认 `base_url` |
| --- | --- | --- |
...
| `code` | string | 提供商标识符 |
| `name` | string | 显示名称 |
| `base_url` | string | 默认 API 地址（无默认值时为 `null`，如 `azure_openai`、`midjourney`、`custom`） |
| `icon` | string | 图标标识符 |

<Callout type="info">
  `openai_responses` 走 OpenAI Responses API（而非 Chat Completions），`typesafe` 用于 `decision` 类型模型，`custom` 表示任意的 OpenAI 兼容端点。
</Callout>
...
`decision` 是**非生成式**的类型化决策模型（choice/score/yes-no 判定），只能由 `typesafe` 提供商提供；用其他提供商创建 `decision` 模型会返回 `1001 model_type_not_supported`。
...
| `model_type` | string | 模型类型：`chat`、`embedding`、`rerank`、`decision`、`tts`、`stt`、`audio_generation`、`text_to_image`、`text_to_video` |
...
| `model_type` | string | 是 | 模型类型；`decision` 必须搭配 `typesafe` 提供商，`embedding` 也仅支持特定提供商，违反时返回 `1001 model_type_not_supported` |
...
<Callout type="info" title="decision 模型的测试行为">
  当 `model_type` 为 `decision` 时，两个测试端点都会构造一个内置的判定请求（`state` 为一段客服场景描述，`questions` 中有一个 `noul` 类型的问题），通过 Decision 适配器调用提供商；只有返回非空答案才算成功，否则返回 `1001 model_test_empty_decision_result`。`decision` 只支持 `typesafe` 提供商，其他提供商会先被 `1001 model_type_not_supported` 拒绝。
</Callout>

<Callout type="warn">
  `POST /api/v1/admin/models/test` 会在测试前做 endpoint 策略校验（SSRF 白名单）。自定义 `base_url` 不在允许列表内时会直接被拒绝，而不是发起请求。
</Callout>
...
需要 `admin:model:create` 权限。列出提供商暴露的模型，不持久化提供的密钥。
...
| `google` | `/v1beta/models` | 查询参数 `key=<api_key>` |
...
| `ollama` | `/api/tags` | 无需密钥 |
| `typesafe` | 见下方说明 | `Authorization: Bearer <api_key>` |

`typesafe` 的路径基于配置的 `base_url` 拼接：若 `base_url` 的路径以 `/v1` 结尾，则在**该路径后追加** `/models`（即 `<配置路径>/models`）；若 `base_url` 还有其他非空路径，则把完整路径 `/v1/models` 接到其后；否则使用默认的 `/v1/models`。因此 TypeSafe 的发现请求会打到**配置的基础路径**下，而不是固定的 `/v1/models`。

不在支持列表中的提供商会返回 `model_discovery_not_supported`；`base_url` 非法或不在 SSRF 允许列表内分别返回 `model_discovery_base_url_invalid`、`model_endpoint_not_allowlisted`；需要密钥但未提供时返回 `model_discovery_api_key_required`。失败时 HTTP 状态仍为 `200`，但 `data.success` 为 `false`。
