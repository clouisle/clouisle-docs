...
<Callout type="warn">
  ...
  Access control: superusers are allowed; a `private` agent is readable only by its creator (creator-less legacy agents fall back to team scope); non-private agents require team access. Denied requests return `403`, missing resources `404`.
</Callout>
