...
The sandbox artifact endpoint accepts a JWT or `clou_` API key. Internal sandbox requests may instead use timestamp and signature headers. Its size limit is controlled by `SANDBOX_ARTIFACT_MAX_FILE_SIZE_MB`.
