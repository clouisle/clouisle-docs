...
| `run_end` | 每次运行**恰好一个**终止事件，状态为 `completed`、`stopped`、`failed` 或 `interrupted` |
...
| `search` | string | 否 | - | 按**会话标题**模糊匹配（不含消息正文） |
...
平台会话路由器**没有**按消息删除的端点；删除单条消息请使用 agents 路由器下的用户端点：

```
DELETE /api/v1/agents/conversations/{conversation_id}/messages/{message_id}
```

该端点需要 `conversation:delete`，且消息必须属于调用者的会话（详见 [Agent API 的 Agent 会话](./agents#删除单条消息)）。
