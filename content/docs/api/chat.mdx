...
| `file_urls` | array | 否 | 原始上传的 Asset 元数据（`asset_id`, `url`, `filename`） |
...
额外的运行事件：`run_start`（运行身份/状态）、`run_status`（queued/running/stopping 转换）、`input_accepted`（排队的引导/追问已提交）、`run_end`（恰好一个终止事件：`completed | stopped | failed | interrupted`）。
