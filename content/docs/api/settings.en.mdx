...
The optional `category` filter accepts values such as `general`, `security`, `email`, `storage`, `notification`, and `memory`. A single-setting response contains `key`, `value`, `value_type`, `category`, `description`, and `is_public`.
...
The `memory` category controls optional background extraction from completed conversations. The Agent must also have memory enabled; disabling either the global switch or the Agent memory feature prevents extraction tasks. The administration page is `/site-settings/memory`.
...
Notification test endpoints include `test-email`, `test-dingtalk`, `test-wechat`, `test-feishu`, `test-webhook`, and `test-slack`. `test-email` accepts `{ "to_email": "admin@example.com" }`; the other test endpoints take no body.
...
The start endpoint returns a background task ID. Use the status endpoint to inspect the archive task.
