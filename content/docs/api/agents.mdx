...
| `rag_mode` | string | RAG 模式：`off`、`auto`、`agentic`（默认） |
| `variables` | array | 聊天输入变量定义 |
| `knowledge_bases` | array | 关联的知识库配置（含 `retrieval_top_k`、`score_threshold`、`search_mode`） |
...
| `rag_mode` | string | 否 | RAG 模式：`off`、`auto`、`agentic`（默认 `agentic`） |
| `knowledge_base_configs` | array | 否 | 知识库配置（`knowledge_base_id`、`retrieval_top_k`、`score_threshold`、`search_mode`） |
...
| `images` | array | 否 | 视觉图片（`name`、`url`、`type`） |
| `files` | array | 否 | 已解析文件（已弃用，使用 `file_urls`） |
| `file_urls` | array | 否 | 已上传资源的元数据（`asset_id`、`url`、`filename`） |
