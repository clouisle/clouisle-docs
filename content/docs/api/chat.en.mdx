...
Additional run events: `run_start` (run identity/status), `run_status` (queued/running/stopping transitions), `input_accepted` (a queued steering/follow-up was committed), `run_end` (exactly one terminal event with `completed | stopped | failed | interrupted`).
