...
- **路由**：`/chat/[id]`、`/chat/[id]?conversation=[conversationId]`、`/run/[id]?type=agent`、`/run/[id]?type=workflow`、`/run/[id]?type=workflow&run=[runId]`、`/embed/agent/[id]`、`/app/apps/[id]/api`、`/app/apps/[id]/monitor`
- **地标**：`New Chat`、`No conversations yet`、`Rename Conversation`、`Delete Conversation`、`Where should we start?`、`Type a message...`、`Send`、`Stop`、`Attach file`、`Fill in Variables`、`Start Chat`、`Open link?`、`This link was generated in the chat. Please confirm before opening it.`
...
- **错误/访问场景**：尝试发布不完整的图；发布前尝试非调试运行；无应用访问权限访问；未配置的 Answer 节点显示 `Click to configure output variables`、Condition 节点显示 `Click to configure condition`
...
- **正常场景**：授权管理员导航到代表性仪表盘页面；验证平台记忆图的加载、空与填充状态；在 `/users` 验证用户列表、搜索/筛选、分页、创建/编辑/状态变更/批量删除；在 `/site-settings/security` 使用可销毁值练习注册/审批/邮箱验证/账户删除/密码策略/TOTP/会话/模型端点白名单
