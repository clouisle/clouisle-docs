...
<Callout type="info" title="Image needed: chunk preview">
  File: `/images/knowledge-chunk-preview.png`. Screenshot content: the document chunk preview panel with the chunk size, overlap, custom separators, clean text toggle, and chunk result statistics highlighted. Crop the screenshot to the relevant area and outline key actions with the theme color. The future image's `alt` and caption will both use "Chunk preview".
</Callout>
