...
Administrators can configure background extraction for pending user turns under **Site Settings** > **Memory** (`/site-settings/memory`). Extraction queues after the configured cooldown or immediately at the pending-turn threshold. The Agent must also have memory enabled; disabling either switch prevents extraction tasks.
