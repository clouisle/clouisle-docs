...
- **Stop**: Select **Stop** during streaming. The message status is recorded as manually stopped.
- **Edit**: Editing a historical user message creates a new branch from that message and streams a new reply.
- **Regenerate**: Regenerating an assistant message creates a new version. Switching versions only changes the active branch; it never deletes older versions.
- **Delete**: Deleting a message also handles its downstream branches; deleting a conversation permanently deletes all of its messages.
