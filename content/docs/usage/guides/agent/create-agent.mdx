...
通过统计端点 `GET /api/v1/agents/{agent_id}/stats` 获取 Agent 在指定时间段内的指标（`period`: `24h`、`7d`、`30d`、`all`）：

| 指标 | 端点 | 说明 |
| --- | --- | --- |
| 会话与消息数 | `/stats` | 总会话数、总消息数、活跃用户数 |
| Token 用量 | `/stats` | `prompt_tokens`、`completion_tokens`、`total_tokens` |
| 平均响应时间 | `/stats` | 端到端平均耗时 |
| 工具调用 | `/stats/tool-usage` | 各工具调用次数统计 |
| 使用趋势 | `/stats/trends` | 按时间聚合的使用量变化 |
| 最近会话 | `/stats/recent-conversations` | 最近的对话列表 |

<Callout type="info">
  统计指标在 Agent 列表/详情中暴露，没有单独的**统计**标签页。
</Callout>

<Callout type="warn">
  未实现/路线图：逐 Agent 成本拆分、响应时间百分位数（p50/p95/p99）、统计导出（CSV/PDF）、定时使用报告。统计端点仅返回上述指标。
</Callout>
