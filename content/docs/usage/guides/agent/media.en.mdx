...
Attachments give the Agent user assets; media generation lets the Agent call authorized image or video models. Both must be explicitly enabled in the Agent orchestration.

<Callout type="info">
  Media generation covers **images and videos only** — no audio generation or speech synthesis. When a switch is on, the platform injects the `generate_image` / `generate_video` tool into the model; when it is off, the model never sees those tools.
</Callout>
...
4. The status polling interval defaults to `3000ms` (range `500-30000ms`) and the synchronous wait cap defaults to `120` seconds (range `5-600`). Once the synchronous cap is exceeded, the task may come back as still-processing rather than failed — retrieve it later via the status tool or `GET /api/v1/agents/{agent_id}/media/video-status`.
