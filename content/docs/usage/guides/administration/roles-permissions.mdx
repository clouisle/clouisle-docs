...
<Callout type="info" title="图片待补充：角色与权限管理">
  文件：`/images/roles-permissions.png`。截图内容：角色详情页展示角色名称、系统/自定义状态、权限列表和保存操作。截图需裁剪到相关区域，并用主题色框线标出关键操作。未来图片的 `alt` 与 caption 均使用“角色与权限管理”。
</Callout>
