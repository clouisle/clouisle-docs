...
When Agent memory is enabled, the model can read the user's memory graph and write memories when permitted. Read tools remain available when automatic extraction is disabled; write tools are gated by the Agent memory configuration.

| Tool | Purpose | Limits |
| --- | --- | --- |
| `search_memory` | Find relevant memory entities | `top_k` is clamped to `1-10`; supports time-window and entity-type filters |
| `get_memory_subgraph` | Read related entities and relationships | At most 5 seeds, depth `1-3`, direction `incoming`, `outgoing`, or `both` |

Use `search_memory` to find candidate entities first; call `get_memory_subgraph` only when the answer depends on relationships.
