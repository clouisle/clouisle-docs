...
<Callout type="info" title="Image needed: service logs and retrieval diagnostics">
  File: `/images/troubleshooting-logs-and-diagnostics.png`. Screenshot content: terminal service logs and retrieval lab diagnostics shown side by side; mark the first failure, error category, and recovery suggestions, and hide sensitive content. Crop the screenshot to the relevant area and outline key actions with the theme color. The future image's `alt` and caption both use "service logs and retrieval diagnostics".
</Callout>
