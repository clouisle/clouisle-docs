...
<Callout type="info" title="Image needed: backup and recovery checklist">
  File: `/images/backup-restore-checklist.png`. Screenshot content: the operations panel or a terminal showing backup files, the database/Qdrant/upload recovery order, and health check results; hide real credentials. Crop the screenshot to the relevant area and outline key actions with the theme color. The future image's `alt` and caption both use "backup and recovery checklist".
</Callout>
