...
<Callout type="info" title="Image needed: Clouisle observability dashboard">
  File: `/images/observability-dashboard.png`. Screenshot content: the admin panel observability page showing API/task status, conversation and Workflow metrics, failure trends, and time-range filtering. Crop the screenshot to the relevant area and outline key actions with the theme color. The future image's `alt` and caption both use "Clouisle observability dashboard".
</Callout>
