...
任务进了队列但没人消费，说明消费该队列的 Worker 命令/部署不对。最典型的是 `worker` 的 `-Q` 少了 `agent`：持久化 AgentRun 全部堆在 `agent` 队列，会话一直停在运行中且不报错。队列与任务的完整对应关系见[部署架构 → Celery 队列划分](../architecture.mdx#celery-队列划分)。
