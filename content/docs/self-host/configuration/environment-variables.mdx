...
`DB_AGGREGATE_CONCURRENCY` 限制的是一个 API 进程内所有聚合 fan-out 共用的信号量，用于防止单次工作台统计请求占满 Tortoise 共享连接池（默认 `maxsize=5`）后把自己的其余查询排在自己后面。仪表盘、Agent 与工作流统计出现连接等待或超时时可下调；把连接池整体调大不是等价方案，默认部署约 17 个进程 × 池，PostgreSQL `max_connections` 为 100。取值与验证方法见[数据库维护与索引调优](../operations/database-maintenance.mdx)。

Worker 消费的队列不是环境变量，而是启动命令的 `-Q` 参数，见[部署架构 → Celery 队列划分](../architecture.mdx#celery-队列划分)。
