...
<Callout type="info" title="图片待补充：Kubernetes 资源与 Ingress">
  文件：`/images/kubernetes-resources.png`。截图内容：Kubernetes namespace 中 API、Worker、Sandbox Worker、Beat、Frontend、PostgreSQL、Redis、Qdrant 和 Ingress 资源关系。截图需裁剪到相关区域，并用主题色框线标出关键操作。未来图片的 `alt` 与 caption 均使用“Kubernetes 资源与 Ingress”。
</Callout>
